...
const JsonViewer = () => {
                ...
                <div className="mt-2 mb-6 p-4 bg-white shadow rounded-lg text-left border-l-4 border-pink-300">
                    ...
                    {Array.isArray(value) ? (
                        value.map((item, index) => (
                            <div key={index} className="p-2 bg-purple-100 rounded-lg my-2 border border-purple-300">
                                {typeof item === 'object' && item !== null ? displayKeyValue(Object.keys(item)[0], item[Object.keys(item)[0]]) : <p className="italic">{item}</p>}
                            </div>
                        ))
                    ) : (
                        Object.keys(value).map(subKey => displayKeyValue(subKey, value[subKey]))
                    )}
                </div>
            );
        } else {
            return (
                <p className="italic">{JSON.stringify(value, null, 2)}</p>
            );
        }
    };
    ...
    const pageContent = () => {
        ...
        return keys.slice(start, end).map(key => displayKeyValue(key, jsonData[key]));
    };
    ...
    const setupPagination = () => {
        ...
        return [...Array(pageCount).keys()].map(i => (
            <button key={i} className={`px-4 py-2 rounded-full ${currentPage === i + 1 ? 'bg-blue-400 text-white' : 'bg-yellow-200 text-gray-700'} hover:bg-blue-300`}
    ...
    return (
        <div className="container mx-auto px-4 py-10 bg-gradient-to-r from-green-100 to-blue-100 font-sans leading-normal tracking-normal">
            ...
            <div className="flex justify-center space-x-2 mt-4">{setupPagination()}</div>
        </div>
    );
};
